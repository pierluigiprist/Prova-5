...
TECNICI_DEFAULT = ["Pierluigi", "Yarema", "Gioele", "Terry", "Christian", "Manuel"]
...
# Fase 2: Selezione tecnico
    ...
    tec_sel = st.selectbox("Tecnico", TECNICI_DEFAULT)
...
with st.sidebar:
    ...
    if st.session_state.get("tecnico_loggato"):
        ...
        if st.button("🚪 Logout", use_container_width=True):
            for key in ["authenticated", "tecnico_loggato", "password_ok", "data"]:
...
if page == "Cruscotto KPI":
    ...
    if not df_bom.empty:
        def barra_livello(row):
            cap = row.get("cap_kg", 0)
            qta = row.get("qta_presente", 0)
            tipo = row.get("tipo_bombola", "Cariche")
            if cap <= 0:
                return "N/D"
            ...
            bar = "█" * filled + "░" * empty
            if tipo == "Cariche":
                color = "#2E7D32" if pct > 50 else "#F9A825" if pct > 20 else "#C62828"
            else:
                color = "#C62828" if pct > 80 else "#F9A825" if pct > 50 else "#2E7D32"
            return f'<span style="color:{color};font-weight:700;">{bar}</span> <span style="font-size:0.85rem;">{pct:.0f}% ({qta:.2f}/{cap:.1f} kg)</span>'
        df_bom["Livello"] = df_bom.apply(barra_livello, axis=1)
        st.dataframe(
            df_bom[["tipo_gas", "id_interno", "seriale", "Livello", "tipo_bombola", "tara", "cap_kg", "stato", "in_carico_a", "note"]],
            use_container_width=True, hide_index=True, column_config={
                "Livello": st.column_config.TextColumn("Livello Gas", width="large")
            }
        )
...
if page == "Report Movimentazioni":
    ...
    if not movs:
        ...
        if not df_filt.empty:
            st.dataframe(
                df_filt[["data", "tipo_gas", "id_bombola", "tipo_mov", "quantita", "cliente", "tecnico", "stoccaggio", "note"]],
                use_container_width=True, hide_index=True
            )
            colori = get_colori_gas()
            html_rows = ""
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Quantità</th><th>Cliente</th><th>Tecnico</th><th>Stoccaggio</th><th>Note</th>
                        </tr>
                    </thead>
