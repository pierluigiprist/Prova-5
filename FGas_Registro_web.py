...
def decodifica_qr_immagine(image_bytes):
    """Decodifica un QR code da bytes di immagine."""
...
if page == "Scanner QR":
    ...
    with tab_cam:
        if not CV2_AVAILABLE:
            st.warning("⚠️ OpenCV non installato. Usa la scheda 'Inserisci Testo QR' oppure aggiungi `opencv-python` al requirements.txt")
        else:
