...
def genera_excel_movimentazioni():
    """Costruisce in memoria un file .xlsx con lo storico movimentazioni, pronto per Drive/LibreOffice Calc."""
    movs = st.session_state.data.get("movimentazioni", [])
    colonne = ["data", "tipo_gas", "id_bombola", "tipo_mov", "quantita", "cliente", "tecnico", "stoccaggio", "note", "origine"]
    intestazioni = ["Data", "Gas", "ID Bombola", "Tipo Movimento", "Quantita (kg)", "Cliente/Zona", "Tecnico", "Stoccaggio", "Note", "Origine"]
    ...
    if movs:
        ...
        for c in colonne:
            if c not in df.columns:
                df[c] = ""
        df = df[colonne].fillna("")
        df.columns = intestazioni
    else:
        df = pd.DataFrame(columns=intestazioni)

    buffer = io.BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Movimentazioni")
        ws = writer.sheets["Movimentazioni"]
        ...
        for col_cells in ws.columns:
            lunghezza = max((len(str(c.value)) if c.value is not None else 0) for c in col_cells)
            ws.column_dimensions[col_cells[0].column_letter].width = min(max(lunghezza + 3, 10), 40)
    buffer.seek(0)
    return buffer.getvalue()

def salva_excel_drive(silent=True):
    """Crea (se non esiste) o aggiorna il file Excel con lo storico movimentazioni su Google Drive."""
    service = get_drive_service()
    if not service:
        if not silent:
            st.toast("Drive non configurato", icon="⚠️")
        return False
    try:
        contenuto = genera_excel_movimentazioni()
        file_info = trova_file_drive(service, EXCEL_FILENAME)
        ...
        if file_info:
            service.files().update(
                fileId=file_info["id"], media_body=media, supportsAllDrives=True
            ).execute()
        else:
            metadata = {"name": EXCEL_FILENAME, "parents": [FOLDER_ID]}
            service.files().create(
                body=metadata, media_body=media, supportsAllDrives=True, fields="id"
            ).execute()
        if not silent:
            st.toast("Foglio Excel aggiornato su Drive", icon="✅")
        return True
    except Exception as e:
        if not silent:
            st.error(f"Errore aggiornamento Excel su Drive: {e}")
        return False
...
with st.sidebar:
    ...
    if st.button("📊 Aggiorna Excel Movimentazioni su Drive", use_container_width=True):
        if salva_excel_drive(silent=False):
            st.success(f"'{EXCEL_FILENAME}' aggiornato su Drive!")
        else:
            st.error("Aggiornamento Excel fallito. Verifica i Secrets.")
    st.download_button(
        label="⬇️ Scarica Excel Movimentazioni",
        data=genera_excel_movimentazioni(),
...
if page == "Cruscotto KPI":
    ...
    if movs:
        ...
        if not df_all.empty:
            ...
            with col_g1:
                st.markdown("**Cariche F-Gas per Mese (kg)**")
                if not cariche.empty:
                    st.bar_chart(cariche, use_container_width=True)
                else:
                    st.info("Nessuna carica registrata")
            with col_g2:
                st.markdown("**Recuperi F-Gas per Mese (kg)**")
                if not recuperi.empty:
                    st.bar_chart(recuperi, use_container_width=True)
                else:
                    st.info("Nessun recupero registrato")
        else:
