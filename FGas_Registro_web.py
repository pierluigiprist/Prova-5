...
# ==================== FUNZIONI QR CODE ====================
def genera_dati_qr(bombola):
    """Genera il payload JSON per il QR code della bombola."""
    return json.dumps({
        "v": "FGas1",
...
def genera_immagine_qr(bombola, size=400):
    """Genera un'immagine PIL del QR code."""
    if not QR_AVAILABLE:
        return None
    dati = genera_dati_qr(bombola)
    qr = qrcode.QRCode(
        version=None,
...
def decodifica_qr_immagine(image_bytes):
    # Prova pyzbar (piu leggero, meglio su cloud)
    if PYZBAR_AVAILABLE:
        try:
...
def genera_html_stampa_qr(bombole):
    """Genera HTML per la stampa dei QR code (una bombola per pagina A4)."""
    if not QR_AVAILABLE:
        return ""
    rows = ""
    for b in bombole:
        img = genera_immagine_qr(b, size=400)
        if img:
            buf = io.BytesIO()
...
def render_form_da_qr(payload):
    """Mostra il form precompilato per movimentazione da QR code."""
...
if page == "Genera QR Code":
    ...
    if len(selezionate) > 0:
        if st.button("🖨️ Genera QR Code per Stampa", use_container_width=True):
            ...
            html_stampa = genera_html_stampa_qr(bombole_obj)
            st.download_button(
...
if page == "Scanner QR":
    ...
    st.markdown('<div class="sub-header">Scansiona il QR code di una bombola per registrare rapidamente una movimentazione</div>', unsafe_allow_html=True)
    st.divider()

    tab_cam, tab_txt = st.tabs(["📷 Fotocamera", "⌨️ Inserisci Testo QR"])

    payload = None

    with tab_cam:
        if not PYZBAR_AVAILABLE and not CV2_AVAILABLE:
            st.warning("⚠️ Nessuna libreria di decodifica QR trovata.\n\n"
                       "**Soluzione 1 (consigliata):** aggiungi `pyzbar` al `requirements.txt` e rimuovi `opencv-python`.\n\n"
                       "**Soluzione 2:** sostituisci `opencv-python` con `opencv-python-headless` nel `requirements.txt`.\n\n"
                       "Nel frattempo puoi usare la scheda **'Inserisci Testo QR'**.")
        else:
            img_file = st.camera_input("Inquadra il QR code della bombola")
            if img_file is not None:
                dati = decodifica_qr_immagine(img_file.getvalue())
                if dati:
                    try:
                        p = json.loads(dati)
                        if p.get("v", "").startswith("FGas"):
                            payload = p
                            st.success("✅ QR Code decodificato correttamente!")
                        else:
                            st.error("❌ QR Code non valido per questo sistema FGas.")
                    except Exception:
                        st.error("❌ Il contenuto del QR non è leggibile come JSON.")
                else:
                    st.error("❌ Nessun QR code rilevato nell'immagine. Riprova con più luce e tenendo fermo il telefono.")

    with tab_txt:
        testo_qr = st.text_area("Incolla qui il contenuto del QR code (testo JSON)", height=120)
        if testo_qr.strip():
            try:
                p = json.loads(testo_qr.strip())
                if p.get("v", "").startswith("FGas"):
                    payload = p
                    st.success("✅ Testo QR valido!")
                else:
                    st.error("❌ Formato QR non riconosciuto.")
            except Exception:
                st.error("❌ Il testo incollato non è un JSON valido.")
    ...
    if payload:
        ...
        if bombola_trovata:
            ...
            st.error(f"⚠️ Bombola {payload['id']} ({payload['gas']}) non trovata nell'anagrafica attuale. Potrebbe essere stata eliminata o l'ID è cambiato.")
