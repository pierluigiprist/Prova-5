...
if st.session_state.get("password_ok") and not st.session_state.get("tecnico_loggato"):
    ...
    tec_list = [t["nome"] for t in st.session_state.data.get("tecnici", [])]
    if tec_list:
...
# Migrazione colori: forza i colori richiesti dall'utente
...
if page == "Cruscotto KPI":
    ...
    if movs:
        ...
        if not df_all.empty:
            ...
            # Raggruppa per mese e tipo_mov
            df_all["anno_mese"] = df_all["dt"].dt.strftime("%Y-%m")
            fig, axes = plt.subplots(1, 2, figsize=(14, 4.5))
            # Grafico 1: Cariche per gas
